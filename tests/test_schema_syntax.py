...
class TestSchemaSyntax(unittest.TestCase):
    def test_no_duplicate_keys_in_any_object(self):
        ...
        self.assertEqual(
            [], duplicates, "duplicate keys inside one object: %r" % duplicates
        )
    ...
    def test_encoding_is_utf8_without_bom_and_lf_only(self):
        with io.open(SCHEMA_PATH, "rb") as f:
            ...
            body = head + f.read()
        self.assertNotEqual(
            b"\xef\xbb\xbf", head, "schema must not start with a UTF-8 BOM"
        )
    ...
    def test_declares_draft_07_and_is_a_valid_schema(self):
        ...
        self.assertEqual(
            "http://json-schema.org/draft-07/schema#", schema.get("$schema")
        )
        ...
        def walk(node):
            if isinstance(node, dict):
                ...
                if isinstance(ref, str):
                    if (
                        not ref.startswith("#/definitions/")
                        or ref.split("/")[-1] not in definitions
                    ):
                        dangling.append(ref)
