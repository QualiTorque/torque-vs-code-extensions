...
export class BlueprintsProvider implements vscode.TreeDataProvider<Blueprint> {
	...
	getChildren(element?: Blueprint): Thenable<Blueprint[]> {
		...
		if (!element)
		{
			var spaces = Array(this.connections.length);
			for (var c=0; c<this.connections.length; c++) {
				var state = vscode.TreeItemCollapsibleState.Collapsed;
				if (this.connections[c]["default"])
					state = vscode.TreeItemCollapsibleState.Expanded;
				spaces[c] = new Space(this.connections[c]["space"], state);
				spaces[c].icon_path = this.connections[c];
			}
			return Promise.resolve(spaces);
		}
		else
			return Promise.resolve(this.getOnlineBlueprints(element));
